...
이번 세션 에서는 :
 - 기존에 일부 만들어진 **`인스턴트 앱`** 을 사용하며,  **"인스턴트 앱 빌더"** 와 그 안에서 사용할 수 있는 UX 기능에 대해 검토 합니다.
...
<table width="50%" border="0">
  ...
  **NOTE**: "인스턴트 앱"을 대화 흐름에 적용하기 위해서는 **_System.Interactive_** 컴포넌트를 Dialog Flow 에서 사용합니다. 해당 컴포넌트는 대화 흐름에서 관련 정보를 **`"인스턴트 앱"`** 으로 전달하고(이번 케이스에서는 사용자의 이름을 전달함) 하고 **`"인스턴트 앱"`** 에서 사용자의 입력이 "Submit" 이 되면 입력된 정보를 대화 흐름에 전달 하게 됩니다.
...
2. 상단 우축에 **`[Instant App]`** 버튼을 클릭하여 **"인스턴트 앱 빌더"** 를 오픈 합니다.

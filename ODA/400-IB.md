...
대화형 인터페이스가 백엔드 프로세스 및 데이터와 자연스러운 상호 작용한 가는 하지만, 많은 입력 데이터를 주고 받아야 하는 경우 질문의 응답의 대화가 더 어렵고 번거려울 수 있다.

각각의 데이터 값을 반환하기 위해 반복적 인  질문에 대한 응답을 요구하기 보다는, 전통적인 양식 스타일 인터페이스를 사용하여 한 번에 데이터를 입력하는 것이 더 효율적이고 직관적입니다.

Oracle Digital Assistant 는 "Instant App" 을 사용하여 이 문제를 해결 할 수 있습니다. 사용자와의 대화에서 조금 더 구체적이고 많은 질의가 필요한 시점에 대화의 흐름에 임베디드 될 수 있는 대화형 위젯 입니다.

"Instant App"을 대화 흐름에 통합하고 표준 UI 요소 (텍스트 필드, 팝 -리스트, 라디오 버튼, 체크 박스 등)를 사용하는 Form 인터페이스를 사용하여 필요한 구조화 된 데이터를 수집 할 수 있습니다. 입력의 과정에서 모든 유효성 검사 통과하고 다시 대화 단계로 다시 전달합니다.


이번 세션 에서는 :
 - 템플릿으로 부터 "Instant App" Builder 와 사용할 수 있는 UX 기능에 대해 검토 합니다.
...
<table width="50%" border="0">
 ...
 - "Instant Apps" 대화 흐름에 적용하기 위해서는 **_System.Interactive_** 컴포넌트를 Dialog Flow 에서 사용하게 됩니다.
    해당 컴포넌트는 대화 흐름에서 관련 정보를 "Instant App" 으로 전달하고(이번 케이스에서는 사용자의 이름을 전달함) 하고 "Instant App" 에서 "Submit" 이되면 입력된 정보를 대화 흐름에 전달 하게 됩니다.

...
### 소개 ###
 ...
 대화 형태의 인터페이스는 챗봇을 통해 Backend 프로세스 또는 데이터와 자연스러운 상호 작용이 가능 하지만, 많은 데이터를 주고 받아야 하는 대화의 경우에는 오히려 질문과 응답의 대화가 더 어렵고 번거러울 수 있습니다. 각각의 데이터 값을 반환하기 위해 반복적 인 질문과 그에 대한 응답을 요구하기 보다는, 이미 우리에게 익숙해 있는 입력 폼 형태의 인터페이스를 사용하면, 여러번의 질의 응답 보다 한 번에 데이터를 입력하는 것이 더 효율적이고 직관적일 수 있습니다.

 이런 문제에 대해 Oracle Digital Assistant에서는 `인스턴트 앱(Instant App)` 을 사용하여 해결 할 수 있습니다. `인스턴드 앱(Instant App)` 은 봇과 사용자와의 대화에서 조금 더 구체적이고 많은 질의가 필요한 시점에 대화의 흐름과 통합될 수 있는 될 수 있는 위젯(Widget) 입니다. **`인스턴트 앱(Instant App)`** 을 대화 흐름에 통합하고 표준 UI 요소 (텍스트 필드, 리스트, 라디오 버튼, 체크 박스 등)를 사용하는 Form 인터페이스를 사용하여 사용자로 부터 구조화 된 데이터를 수집 할 수 있습니다. 또한 사용자의 입력의 과정에서 모든 유효성 검사 가능하며, 입력된 정보를 다시 대화 단계로 다시 전달 할 수 있습니다. 

 결국, **`인스턴트 앱(Instant App)`** 을 통해 대화형 인터페이스에서 발생 할 수 있는 번거로운 대화의 한계를 극복할 수 있습니다.

 이번 세션 에서는 :
  - 기존에 일부 만들어진 인스턴트 앱을 사용하며, 인스턴트 앱 빌더와 그 안에서 사용할 수 있는 UX 기능에 대해 검토 합니다.
...
<table width="50%" border="0">
  ...
  **NOTE**: 인스턴트 앱을 대화 흐름에 적용하기 위해서는 **_System.Interactive_** 컴포넌트를 Dialog Flow 에서 사용합니다. 해당 컴포넌트는 대화 흐름에서 관련 정보를 인스턴트 앱으로 전달하고(이번 케이스에서는 사용자의 이름을 전달함) 하고 인스턴트 앱에서 사용자의 입력이 "Submit" 이 되면 입력된 정보를 대화 흐름에 전달 하게 됩니다.
...
2. 상단 우축에 **`[Instant App]`** 버튼을 클릭하여 인스턴트 앱 빌더를 오픈 합니다.
...
Create App 페이지로 이동하기 위해서 **`[Add Instant App]`** 버튼을 클릭합니다.

<img src="img/lab4-2c.png" width="50%"/>

위와 같이 새로운 "Instant App"을 만들기 위한 다양한 옵션을 볼 수 있습니다.
...
5. 스크롤을 내려서 이미 일부 만들어진 "CafeSupremo : Design My Lunch" 인스턴트 앱을 선택 합니다.
...
6. 인스턴트 앱을 선택하면 아래 이미지와 같이 메뉴가 출력되고 "Edit a Copy" 를 선택 합니다. 선택이 되면, 복제된 인스턴트 앱이 생성됩니다.
...
**`중요`** : 위 이미지에서 APP ID 를 기억해두고나 기록해 두도록 합니다. 해당 APP ID 는 Skill 과 통합할 때 사용됩니다.

8. 현재 패널의 레이아웃 및 UI 구성 요소를 보기 위해서 "“MySandwichOptions”"을 선택합니다. 

<img src="img/lab4-8.png" width="100%"/>

9. 샌드위치에 Dressiong Option 을 추가하도록 하겠습니다. Checkbox 를 빌더(builder)의 왼쪽 판넬의 있는 "MySandwichOptions"의 "Chkbox_Salad" 구성요소 아래오 Drag 합니다.

...
사용자와 챗봇과 대화하는 대화형 인터페이스는 백엔드 프로세스 또는 데이터와 자연스러운 상호 작용이 가능 하지만, 많은 데이터를 주고 받아야 하는 대화의 경우 오히려 질문과 응답하는 형태의 대화가 사용자에게 더 어렵고 번거러울 수 있습니다. 사용자에게 다양하고 많은 반복 질문을 사용자에게 요구하고 그에 대한 각각의 응답 제공하기 보다는, 이미 우리에게 익숙해 있는 입력 폼 형태의 인터페이스를 사용하면, 여러번의 질문과 응답 보다 한 번에 데이터를 입력하는 것이 더 효율적이고 직관적일 수 있습니다.
...
**`NOTE`**: 25%로 사이즈를 조정해야만 새로 만든 CheckBox 가 다른 Check Box 옵션들과 함께 한줄에 들어 갈 수 있습니다.
...
19. 새로 추가 한 "Choose_Dressing_Type" 옵션의 경우 사용자가 앞서 추가한 Dressing CheckBox 를 선택한 경우 화면에 나타나야 합니다. (선택되지 않은 경우에는 사라져야 함)

- 현재 애플리케이션 또는 Element 에 정의된 다양한 Event 들을 보기 위해 "Event and Action" 버튼을 클릭합니다. (현재 애플리케이션에 추가되어 있는 Element)

...
Oracle Digital Assistant 인스턴스가 생성되어 있지 않은 경우, 인스턴스를 생성 하도록 합니다.

1. 오라클 클라우드에 로그인 하고, 대쉬보드 사용자정의(Customized Dashboard) 를 선택합니다.
   
    <img src="images/lab2-0.PNG" width="75%"/>

2. 스크롤 다운을 하여 Digital Assistant 에 표시(Expancd) 를 선택하여 대쉬보드에 추가합니다.
    
    <img src="images/lab2-1.png" width="75%"/>

3. Digital Assistant 를 선택하여, Console 화면으로 이동합니다.

    <img src="images/lab2-2.png" width="75%"/>

4. 서비스 콘솔 열기(Open Console)을 선택합니다.

    <img src="images/lab2-3.png" width="75%"/>

5. 인스턴스 목록 화면에서 `[인스턴스 생성]` 버튼을 선택 합니다.

    <img src="images/instance list.png" width="75%"/>

6. 인스턴스 이름을 입력하고 다음 버튼을 선택하여, 인스턴스를 생성합니다.

    <img src="images/create instance.png" width="75%"/>

### Exercise Steps ###

아래 실습에서는 사전에 정의된 의도(intent)와, 엔티티, 대화 흐름을 구성하고, 자연어 처리 엔진을 학습시켜, 올바르게 동작하는지 테스트하겠습니다.

1. Oracle Digital Assistant 인스턴스 목록에서 `Digital Assistnant Designe UI` 메뉴를 선택하면, Digital Assistant Designer UI 가 보입니다.

    <img src="images/lab1-4.png" width="75%"/>
...
- 다음과 같이 스킬 생성 정보를 입력하고 Create 버튼을 클릭합니다.
  * Display Name : ``"[YOUR INITIALS]"`` "[ ]"안에 본인의 이니셜을 넣습니다.
  * Name : 위와 동일
  * Version : 0.1
...
**NOTE:** 위 메시지를 입력하면, NLP 엔진은 정확하게 어떤 인텐트(intent)인지 알 수 없지만, 어떤 인텐트(intent)와 가까운지 보여줍니다. 

<img src="img/lab2-10.png" width="50%"/>

11. NLP 엔진에 추가 문장을 학습 시키는 방법은 아래와 같습니다. 
- 가장 가까운 **인텐트(intent)**의 Radio 버튼을 선택합니다.(위 메시의 경우는 **"OrderFood"** 입니다.) **``[Add Example]``** 버튼을 선택합니다. 

- [Train] 버튼을 다시 선택합니다. 추가로 학습이 필요한 경우 [Train]버튼의 경우 **(!)** 가 표시 됩니다. 

<img src="img/lab2-11.png"/>

- **`[Submit]`** 버튼을 선택하시 NLP 엔진을 학습 시킵니다.
...
13. `[TryOut]` 을 선택하여 테스터를 종료 합니다.

14. 왼쪽 상단 툴바에서 있는 엔티티(Engities) 아이콘을 선택합니다.(위에서 2번째 위치해 있음) 

<img src="img/lab2-14.png" width="50%"/>

15. `[More]` 버튼을 선택하고  and `[Value List Entities]` 를 선택합니다.
...
18. 인텐트(intent) 탭으로 돌아가서, "OrderFood" 인텐트를 선택하고, 오른쪽 상단에 있는 ``[+Entity]`` 버튼을 선택합니다.

<img src="img/lab2-18.png" width="50%"/>

19. 스크롤 다운을 하면 아래 엔티티 리스트를 할 수 있고, 엔티티의 정의를 선택하고 현재 인텐트(intent) 에 추가 합니다. 
...
20. 테스터를 재시작 합니다. (이전 테스트 기록을 계속해서 보고 싶다면, Restart 를 선택합니다) 
...
24. 디폴트로 입력되어 있는 코드를 삭제 하고 이전에 다운로드 받은 컨텐츠를 붙혀 넣기를 합니다. ``**OrderFoodDialog.txt**`` 

25. "잘라내기 와 붙혀넣기"가 잘 되었는지 Validation 버튼을 클릭하여 확인하세요. 정상적인 경우 아래와 같이 메시지가 표시됩니다.

<img src="img/lab2-25a.png"/>

<img src="img/lab2-25b.png" width="75%"/>

26. 테스터를 재시작 하고 "Bot" 탭을 변경하세요.

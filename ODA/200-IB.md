...
### 소개 ###
오라클 클라우드에서 제공하는 대화형 인터페이스는 더 이상 챗봇(Chat Bot)이라고 하지 않으며, 디지털 어시스턴트(Digital Assistant)라고 합니다. 기존이 이미 잘알려진 챗봇의 경우, 사람과 대화를 위해 프로그래밍된 Bot 으로, 자연어 처리를 기반의 대화형 인터페이스를 제공합니다. 하지만, 일반적으로 챗봇의 경우 단일 목적의 응대를 목표로 합니다. 그래서 다수의 목적을 수행 하기 위한 챗봇을 구현 해야 하는 경우 구조적인 한계를 가지고 있기 때문에 다양한 업무 영역을 적용하기 어렵습니다.

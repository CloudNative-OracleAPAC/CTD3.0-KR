...
### 소개 ###
오라클 클라우드에서 제공하는 대화형 인터페이스 클라우드 서비스를 디지털 어시스턴트(Digital Assistant)라고 합니다. 기존이 이미 잘 알려진 챗봇(Chat Bot)의 경우 사람과의 대화를 위해 프로그래밍된 프로그램으로서, 자연어 처리가 가능한 대화형 인터페이스를 제공합니다. 이러한 챗봇의 경우 일반적으로 단일 목적의 업무를 응대 하는 것을 목표로 합니다. 하지만 다수의 목적을 수행 하기 위한 챗봇을 구현 해야 하는 경우 구조적인 한계로 인해 확장성의 어려움을 가지고 있습니다.

이를 극복한 대화형 인터페이스를 디지털 어시스턴트(Digital Assistant)라고 합니다. DA(Digtal Assistant)는 2세대 챗봇으로서 각 업무 및 목적에 맞는 스킬(Skill, 봇)을 개발하고, Digital Assistant 에서 필요한 스킬을 구성할 수 있는 확장성과 유연성을 가지고 있습니다. 예를 들어, 임직원 셀프 서비스 디지털 어시스턴트를 개발 하는 경우, 기존에 만들어진 회의실 예약 스킬, 일정관리 스킬, 휴가신청 스킬을 임직원 셀프 서비스 디지털 어시스턴트에 구성하여, 대화형 인터페이스를 제공 할 수 있습니다.  

<img src="images/digtalassistant.png" width="100%"/>

디지털 어시스턴트를 구성하는 스킬은 다음 구성요소를 포함하고 있습니다.
...
**Skill** 의 구성 요소
 ...
 - **Steps or States:** 대화 흐름을 구성하는 각각의 질문과 응답 입니다. 스텝(Step)들은 "컴포넌트"에 의해 구현 됩니다.
 
 - **Dialogue Flow와 Decision Points:** 다이얼로그 플로우에서는 사용자의 질문, 사용자의 응답, Bot의 응답에 따라, 대화의 흐름을 다음 스텝으로 이동시킵니다.
...
오라클 디지털 어시스턴트(Oracle Digital Assistant)는 실 시간으로 자연어 처리 테스트 및 대화 흐름을 테스트 할 수 있는 환경을 제공 합니다. 또한 개발자/운영자들이 사용자의 메시지의 의도 파악을 위한 품질을 향상 시킬 수 있는 튜닝 방법을 제공하며, 이를 통해 사용자의 챗봇의 만족도를 향상 시킬 수 있습니다. 이번 실습에서는 오라클 디지털 어시스턴트(Oracle Digital Assistant)를 이용하여, Cafe Supremo Digital Assistant를 개발하고 사용자가 원하는 메뉴, 스낵, 점심메뉴를 주문 할 수 있도록 개발하게 됩니다. 이를 위해서 실습자는 카페에서 손님이 음식을 주문하는 것과 과련된 다양하고 새로운 Intent와 Entities를 봇에게 학습 시키게 됩니다.
...
위 2개 압축 파일을 다운로드 하고, 원하는 위치에 압축을 해제 합니다.
 
  * Intents-And-Entities.zip
    - GreetingsIntests.csv (인사와 관련인 의도와 표현 정의)
    - OrderFoodIntents.csv (음식 주문에 대한 의도와 표현 정의)
    - OrderFoodEntites.csv (음식 주문에 대한 엔티티)
    - YesNoEntites.csv     (Yes, No 에 대한 엔티티)

 * DalogYAML.zip
   - OrderFoodDialog.txt(음식 주문에 대한 대화 흐름 정의)

### Exercise Steps ###

아래 실습에서는 사전에 정의된 의도(intent)와, 엔티티(Entities), 대화흐름(Dialog Flow)을 구성하고, 학습 통해, Skill이 올바르게 동작하는지 테스트하겠습니다.
...
3.  스킬이 생성되고 아래 화면이 나오면 **``[import Intents]``** 버튼을 선택하여  **GreetingIntents.csv** 을 임포트 합니다. (인사와 관련된 표현)
...
- `Configuration`의 `Type` 항목에서 **“Entity List”** 옵션을 선택하고 아래와 같이  엔티티 리스트를 추가 합니다.

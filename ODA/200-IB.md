...
### 소개 ###
챗봇 대화를 기반으로 하는 AI 를 디자인 할때, 다음과 같은 많은 구성요소를 고려하는 것이 필요합니다.

 - **의도(Intent):** 사용자가 봇(Bot)에게 질의한 목적을 의미 합니다. 예를 들어 은행 업무와 관련된 봇(Bot)에게 "나의 잔액이 얼마 입니까?" 라고 하면 현재 은행 계좌의 잔액을 알기 위해 질의를 한 것입니다. 
  - **엔터티(Entities):** 사용자의 질의의 의도를 파악된 후 해당 기능을 수행 하기 위해 필요한 중요 정보를 포함되며, 사정의 입력한 다양한 표현안에 포함될 수 있습니다. "예를 들어 나의 계좌의 잔고는 얼마 입니까?" 라는 것은 나의 계좌 정보와 관련된 것입을 알 수 있습니다.
 - **Steps or States:** 대화 흐름을 구성하는 각각의 질문과 응답 입니다. 스텝들은 "컴포넌트"에 의해 구현 됩니다.
 - **Dialog Flow and Decision Points:** 다이얼로그 플로우에서는 사용자의 질문, 사용자의 응답, Bot의 응답에 따라, 대화의 흐름을 다른 스텝으로 이동시킵니다.
 - **Variables:**  대화의 과정에서 일어나는 주요 정보를 저장하며, 유지 합니다. 이를 통해 대화 흐름을 유지 할 수 있도록 합니다.

오라클 인텔리전트 봇 플랫픔은 적은 양의 코드로 고객이 웹 브라우저 상에서 챗폿을 개발할 수 있는 지능형 봇 개발 툴 입니다. 이 툴은 고객이 새로운 챗봇을 생성하고 의도(인텐트), 엔티티, 다이얼로그 플로우를 정의하며, 커스텀 컴포넌트와 다른 채널와 연결할 수 있는 설정을 제공합니다. 


<img src="img/lab2-intro-1.png" width="100%"/>

추가로 이 플랫폼 환경은 자연어 처리를 테스트가 가능하며, 실시간으로 다어얼 로그 플로우를 영향을 주는 부분에 대해 테스트 할 수 있습니다. (테스트 인터페이스 제공) 또한 봇 개발자들은 챗봇이 질문의 의도를 이해를 좀 향상 시킬 수 있도록 하기 위한 튜닝 및 이를 통한 사용 경함을 강화 시킬 수 있습니다.  이번 실습에서는 십습자는 오라클 인텔리전트 봇 빌더를 이용하여, Cafe Supremo Bot로 메뉴를 주문하고 스택과 점심도 주문이 할 수 있도록 확장할 하게 될 것입니다. 실습자는 카페에서 손님이 음식을 주문하는 것에 대한 새로운 개념을 많은 새로운 의도와 엔티티를 통해 봇에게 가르치게 됩니다.

### 사전 준비 사항 ###
봇 생성을 조금더 간단히 하기 위해서, 실습자는 사전의 정의된 인텐트와 엔티티 뿐만 아니라, 다이얼로그 플로그 정의에 대해서 임포트 하게 됩니다. 다이얼로그 플로우의 경우 오라클 봇 마크업 언어로 정의됩니다. 아래 링크에 위치해 있는 압축 파일을 다운로드 받으시기 바랍니다. 이 파일들이 실습에 사용됩니다.
...
2. 봇의 이름을 생성하세요. ``"Cafe_Supremo_TD_[YOUR INITIALS]"`` 
 * ``"[YOUR INITIALS]"`` "[ ]"안에 본인의 이니셜을 넣습니다.
 * 설명 입력란에는 **“Cafe Supremo Extension built by [YOUR NAME]"** "[ ]"안에 본인의 이니셜을 넣습니다..

<img src="img/lab2-2.png" width="75%"/>

3.  **``[import Intents]``** 버튼을 선택하여  **GreetingIntents.csv** 파일을 **“Intents and Entities”** 임포트 합니다..
...
5. **``“OrderFoodIntent.csv”``** 파일로 부터 Intent 를 임포트 합니다.

...
**NOTE:** 위 메시지를 입력하면, NLP 엔진은 정확하게 어떤 인텐트(intent)인지 알 수 없지만, 어떤 인텐트(intent)와 가까운지 보여줍니다.  (아래 이미지 수정 필요!!)
...
20. **`[Try Out]`** 선택하여 테스터를 재시작 합니다. (이전 테스트 기록을 계속해서 보고 싶다면, Restart 를 선택합니다) 
